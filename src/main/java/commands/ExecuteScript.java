...
public class ExecuteScript implements CommandInterface{
    ...
    public void execute(String args) throws IOException {
        ...
        for(int i=0; i <= script.length - 1; i++){
            CommandInitializator.scripts.add(String.format("execute_script %s", args));
            if (!(CommandInitializator.scripts.contains(script[i]) || CommandInitializator.scripts.isEmpty() || (CommandInitializator.scripts.contains(script[i]) && (CommandInitializator.scripts.get(CommandInitializator.scripts.toArray().length - 2) == String.format("execute_script %s", args) && CommandInitializator.scripts.get(CommandInitializator.scripts.toArray().length - 1) != String.format("execute_script %s", args))))) {
